                    ...
                    <div class="uni-card">
                        ...
                        <div class="uni-card-header">
                            <a href="{{ route('agent.universities.show', $uni->id) }}">
                                @if ($uni->university_logo)
                                    <img src="{{ asset('storage/uni_logo/' . $uni->university_logo) }}" class="uni-logo-full"
                                        alt="{{ $uni->name }}">
                                @else
                        ...
                        <div class="uni-card-footer">
                            @if ($uni->courses->count())
                                <button class="btn btn-outline-secondary" onclick="openCourseModal({{ $uni->id }})">
                                    <i class="fas fa-book-open"></i> {{ $uni->courses->count() }} Courses
                                </button>
                            @endif

                            <a href="{{ route('agent.applications.create') }}?university_id={{ $uni->id }}"
                ...
                <div id="courseModal{{ $uni->id }}" class="uni-modal">
                    <div class="uni-modal-content">
                        ...
                        <h4 class="uni-modal-title m-2">
                            <i class="fas fa-book"></i> Courses at <a
                                href="{{ route('agent.universities.show', $uni->id) }}" class="uni-name-link">
                            ...
                            <table class="uni-table">
                                ...
                                <tbody>
                                    @foreach ($uni->courses as $course)
                                        <tr>
                                            <td><a
                                                    href="{{ route('agent.courses.show', $course->id) }}">{{ $course->course_code }}</a>
                                            </td>
                                            <td><a
                                                    href="{{ route('agent.courses.show', $course->id) }}">{{ $course->title }}</a>
                                            </td>
                                            ...
                                            <td>
                                                <a href="{{ route('agent.applications.create') }}?course_id={{ $course->id }}"
                                                    class="uni-btn-success-small">
