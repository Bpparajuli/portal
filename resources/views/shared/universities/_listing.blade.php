@php
    ...
    $filterCourseRoute = route($ajaxPrefix . '.get-courses-by-type', [
        'universityId' => ':universityId',
        'type' => ':type',
    ]);
    $filterFormAction =
        $prefix === 'guest'
            ? route('guest.universities.index')
            : ($prefix === 'staff'
                ? route('staff.universities')
                : route($prefix . '.universities.index'));
        ...
        <div class="row g-3">
            <div class="col-md-3 col-6">
                <div class="uni-listing-stat-card">
                    <div class="uni-listing-stat-icon" style="background:linear-gradient(135deg,#6d28d9,#8b5cf6)"><i
            ...
            <div class="col-md-3 col-6">
                <div class="uni-listing-stat-card">
                    <div class="uni-listing-stat-icon" style="background:linear-gradient(135deg,#059669,#34d399)"><i
            ...
            <div class="col-md-3 col-6">
                <div class="uni-listing-stat-card">
                    <div class="uni-listing-stat-icon" style="background:linear-gradient(135deg,#d97706,#fbbf24)"><i
                            class="fas fa-smile"></i></div>
            ...
            <div class="d-flex justify-content-between align-items-center">
                ...
                @auth
                    @if (in_array($prefix, ['admin', 'staff']))
                        <div>
                            <a href="{{ route($prefix . '.universities.create') }}" class="btn btn-success btn-sm">+ Add
                                University</a>
                            <a href="{{ route($prefix . '.courses.create') }}" class="btn btn-success btn-sm">+ Add
                                Course</a>
                        </div>
                    @endif
                @endauth
                        ...
                        <div class="row g-3">
                            <div class="col-md-2">
                                ...
                                <select id="country" class="form-select" name="country"
                                    data-cities-url="{{ $filterCityRoute }}">
                            ...
                            <div class="col-md-2">
                                ...
                                <select id="city" class="form-select" name="city"
                                    data-universities-url="{{ $filterUniRoute }}">
                                    <option value="">All</option>
                                </select>
                            </div>
                            <div class="col-md-2">
                            ...
                            <div class="col-md-2">
                                ...
                                <select id="course_type" class="form-select" name="course_type"
                                    data-courses-url="{{ $filterCourseRoute }}">
                                    <option value="">All</option>
                                </select>
                            </div>
                            <div class="col-md-2">
                    ...
                    <div class="uni-card-content">
                        ...
                        <div class="uni-badges">
                            ...
                            <span class="badge bg-secondary">
                                <i class="fas fa-book me-1"></i> {{ $u->courses_count ?? $u->courses->count() }}
                                Courses
                            </span>
                        </div>
                        @if ($u->website)
                        ...
                        @auth
                            ...
                            @if ($user->is_admin)
                                <div class="mt-2 d-flex gap-1">
                                    @can('update', $u)
                                        <a href="{{ route($prefix . '.universities.edit', $u) }}"
                                            class="btn btn-sm btn-outline-warning">
                                            <i class="fas fa-edit"></i> Edit
                                        </a>
                                    @endcan
                                    @can('delete', $u)
                                        <x-confirm-delete action="{{ $prefix }}.universities.destroy" :id="$u->id"
                                            label="Delete" title="Delete {{ $u->name }}?"
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        ...
                                                        @foreach (array_slice($intakes, 0, 2) as $intake)
                                                            <span
                                                                class="badge bg-info me-1">{{ trim($intake) }}</span>
                                                        @endforeach
                                                        @if (count($intakes) > 2)
                                                            <span
                                                    ...
                                                    <td>
                                                        @auth
                                                            ...
                                                            @if ($authUser->is_admin)
                                                                @can('update', $course)
                                                                    <a href="{{ route($prefix . '.courses.edit', $course) }}"
...
<style>
    ...
    .uni-listing-hero-content {
        position: relative;
        z-index: 1;
        text-align: center;
    }

    .uni-listing-hero-title {
        font-size: 1.6rem;
        font-weight: 800;
        color: #fff;
    ...
    .uni-listing-hero-sub {
        font-size: 1.05rem;
        color: rgba(255, 255, 255, 0.8);
        margin: 0;
    }

    .uni-listing-stats-row {
        margin-top: -2rem;
        margin-bottom: 2rem;
        position: relative;
        z-index: 2;
    ...
    .uni-listing-stat-card:hover {
        transform: translateY(-4px);
        box-shadow: 0 8px 24px rgba(0, 0, 0, 0.1);
    }

    .uni-listing-stat-icon {
        width: 48px;
        height: 48px;
        border-radius: 12px;
        display: inline-flex;
    ...
    .uni-listing-stat-icon i {
        font-size: 1.3rem;
        color: #fff;
    }

    .uni-listing-stat-num {
        font-size: 1.6rem;
        font-weight: 800;
        color: #1e293b;
        line-height: 1.2;
    ...
    .uni-listing-stat-label {
        font-size: 0.8rem;
        color: #64748b;
        margin-top: 0.2rem;
    }

    .uni-listing-grid-section {
        padding: 2rem 0 3rem;
    }
...
<script>
    ...
    function openCourseModal(id) {
        new bootstrap.Modal(document.getElementById('courseModal' + id)).show();
    }

    function closeCourseModal(id) {
        const m = bootstrap.Modal.getInstance(document.getElementById('courseModal' + id));
        if (m) m.hide();
    }
</script>
