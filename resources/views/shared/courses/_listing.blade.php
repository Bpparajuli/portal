                ...
                <div class="col-md-3">
                    ...
                    <select name="course_type" class="form-select">
                        ...
                        @foreach ($courseTypes as $ct)
                            <option value="{{ $ct }}" {{ request('course_type') == $ct ? 'selected' : '' }}>
                                {{ $ct }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-2 d-flex gap-2">
                    <a href="{{ url()->current() }}" class="btn btn-outline-secondary flex-fill"><i
                            ...
                            <div class="d-flex align-items-start gap-3 mb-3">
                                @if ($c->university && $c->university->university_logo)
                                    <img src="{{ asset('storage/uni_logo/' . $c->university->university_logo) }}"
                                        alt="{{ $c->university->name }}" class="course-uni-logo border">
                                @else
                                    <div
                                        class="course-uni-logo bg-light d-flex align-items-center justify-content-center border rounded">
                            ...
                            <div class="d-flex flex-wrap gap-2 mb-3">
                                ...
                                @if ($c->duration)
                                    <span class="badge bg-light text-dark border"><i
                                            class="far fa-clock me-1"></i>{{ $c->duration }}</span>
                                @endif
                                @if ($c->fee)
                                    <span class="badge bg-light text-dark border text-truncate"
                                        style="max-width:100%;overflow-wrap:break-word;"><i
                                            class="fas fa-tag me-1"></i>{{ $c->fee }}</span>
                                @endif
                            </div>
                            @if ($c->description)
                                <p class="card-text small text-muted flex-grow-1"
                                    style="font-size:0.8rem;line-height:1.5;">{{ Str::limit($c->description, 120) }}
                                </p>
                            @endif
                            @auth
                                ...
                                @if ($user->is_admin)
                                    <div class="d-flex gap-1 mb-2 flex-wrap">
                                        <a href="{{ route($prefix . '.courses.edit', $c) }}"
                                            class="btn btn-sm btn-outline-warning">
                                            <i class="fas fa-edit"></i> Edit
                                        </a>
                                        @can('delete', $c)
                                            <x-confirm-delete action="{{ $prefix }}.courses.destroy" :id="$c->id"
                                                label="Delete" title="Delete {{ $c->course_code }}?"
                                ...
                                @endif
                            @endauth
                            <a href="{{ route($prefix . '.courses.show', $c->id) }}"
