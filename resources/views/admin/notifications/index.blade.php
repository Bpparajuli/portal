...
<div class="notif-page">
    ...
    <div class="notif-header">
        ...
        <div class="d-flex gap-2">
            ...
            <button class="btn btn-sm btn-outline text-danger" onclick="clearAll()">
    ...
    <div class="notif-list" id="notifList">
        @php
            ...
            foreach ($notifications as $n) {
                ...
                $allItems->push([
                    ...
                    'url' => $data['link'] ?? (!empty($data['application']['id']) ? route('admin.applications.show', $data['application']['id']) : (!empty($data['student']['id']) ? route('admin.students.show', $data['student']['id']) : (!empty($data['agent']['id']) ? route('admin.users.show', $data['agent']['id']) : '#'))),
            ...
            foreach ($messages as $m) {
                ...
                $allItems->push([
                    ...
                    'url' => !empty($data['application']['id']) ? route('admin.applications.show', $data['application']['id']) : '#',
        ...
        @forelse($allItems as $item)
            <div class="notif-item {{ $item['is_unread'] ? 'unread' : '' }}" data-type="{{ $item['type'] }}" data-unread="{{ $item['is_unread'] ? '1' : '0' }}">
                ...
                <div class="notif-content">
                    ...
                    <div class="notif-actions">
                        @if($item['url'] && $item['url'] !== '#')
                            <a href="{{ route('admin.notifications.markRead', $item['id']) }}?redirect={{ urlencode($item['url']) }}" class="btn btn-xs btn-primary">View</a>
                        @endif
...
<script>
    ...
    async function markAllRead() {
        ...
        form.method = 'POST';
        form.action = '{{ route("admin.notifications.markAll") }}';
    ...
    async function clearAll() {
        ...
        if (result.isConfirmed) {
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = '{{ route("admin.notifications.deleteAll") }}';
            form.innerHTML = '<input type="hidden" name="_token" value="{{ csrf_token() }}"><input type="hidden" name="_method" value="DELETE">';
            document.body.appendChild(form);
            form.submit();
        }
