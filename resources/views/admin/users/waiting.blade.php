...
@section('content')
                    ...
                    <table class="table table-bordered align-middle text-center">
                        ...
                        <tbody>
                            @foreach ($waitingUsers as $user)
                                <tr>
                                    <td>{{ $user->id }}</td>
                                    <td><a href="{{ route('admin.users.show', $user->slug) }}">{{ $user->business_name }}</a>
                                    </td>
                                    <td><a href="{{ route('admin.users.show', $user->slug) }}">{{ $user->owner_name }}</a>
                                    </td>
                                    <td>{{ $user->email }}</td>
                                    <td>{{ $user->contact }}</td>
                                    <td>{{ $user->address }}</td>
    ...
    =========================== --}}
            ...
            @else
                ...
                <form id="reminderForm" action="{{ route('admin.reminder.send') }}" method="POST"
                    onsubmit="return confirmReminder(this, event)">
                    @csrf
                            ...
                            <tbody>
                                @foreach ($agreementUsers as $user)
                                    <tr>
                                        ...
                                        <td>{{ $user->id }}</td>
                                        <td> <a
                                                href="{{ route('admin.users.show', $user->slug) }}">{{ $user->business_name }}</a>
                                        </td>
                    ...
                    <button type="submit" class="btn btn-primary mt-2" name="send_all" value="0">Send Reminder to
                        Selected Users</button>
                    <button type="submit" class="btn btn-secondary mt-2" name="send_all" value="1">Send Reminder to
                        All Users</button>
                </form>
            @endif
        @else
...
============================= --}}
    <script>
        ...
        document.getElementById('selectAll').addEventListener('click', function() {
            ...
            document.querySelectorAll('#reminderForm input[name="user_ids[]"]').forEach(el => el.checked = this
                .checked);
        });
