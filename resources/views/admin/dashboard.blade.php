            ...
            <div class="col-12">
                <div class="action-card d-flex align-items-center justify-content-between">
                    ...
                    <div class="d-flex gap-2">
                        <a href="{{ route('admin.universities.create') }}" class="btn primary btn-sm"><i
                                class="fas fa-university me-1"></i> Add University</a>
                        <a href="{{ route('admin.courses.create') }}" class="btn secondary btn-sm"><i
                                class="fas fa-book-open me-1"></i> Add Course</a>
                        <a href="{{ route('admin.users.create') }}" class="btn success btn-sm"><i
                                class="fas fa-user me-1"></i> Add User</a>
                        <a href="{{ route('admin.applications.index') }}" class="btn warning btn-sm"><i
                                class="fas fa-tools me-1"></i> Manage Applications</a>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="progress-stats grid-two-columns mt-3">
                            @foreach ($statusChartData['labels'] as $i => $s)
                                <div class="stat"
                                    style="background-color: {{ $statusChartData['datasets'][0]['backgroundColor'][$i] }}; color: #fff; font-weight: bold;">
                                    {{ $s }}
                                </div>
                            @endforeach
                        </div>
                                ...
                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                    <div class="d-flex align-items-center gap-2">
                                        @if ($agent->business_logo)
                                            <img src="{{ Storage::url($agent->business_logo) }}" class="rounded-circle"
                                                width="40" height="40">
                                        @endif
                                        <span>{{ $agent->name }}</span>
                                    </div>
                                    <span class="badge bg-primary rounded-pill">{{ $agent->applications_count }}
                                        Apps</span>
                                </li>
                            @empty
                        ...
                        <tbody>
                            @forelse($latestApplications as $app)
                                <tr>
                                    ...
                                    <td>
                                        <a
                                            href="{{ route('admin.students.show', $app->student->id) }}"class="text-decoration-none fw-semibold">
                                            {{ $app->student->first_name ?? 'N/A' }}
                                            {{ $app->student->last_name ?? 'N/A' }}
                                        </a>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="d-flex gap-1">
                                            <a href="{{ route('admin.applications.show', $app->id) }}"
                                                class="p-2 btn btn-sm primary">View</a>
                                            <a href="{{ route('admin.applications.edit', $app->id) }}"
                                                class="p-2 btn btn-sm secondary">Edit</a>
                                        </div>
                                    </td>
                ...
                <div class="card p-3 h-100">
                    ...
                    <ul class="list-unstyled m-0">
                        ...
                        @forelse ($studentActivities as $act)
                            <a href="{{ $act->link }}"
                                class="activity-link d-flex justify-content-between align-items-center">
                                <li class="mb-3">
