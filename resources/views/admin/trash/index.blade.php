    ...
    <div class="card border-0 shadow-sm rounded-4 mb-4">
        <div class="card-header bg-white border-bottom py-3 d-flex align-items-center gap-2">
            ...
            <div class="d-flex gap-2 align-items-center flex-wrap">
                ...
                <button type="button" class="btn btn-sm btn-outline-success" id="restoreSelectedBtn-{{ $type }}" disabled onclick="bulkAction('{{ $type }}', 'restore')"><i class="fas fa-undo me-1"></i>Restore Selected</button>
                <button type="button" class="btn btn-sm btn-outline-danger" id="deleteSelectedBtn-{{ $type }}" disabled onclick="bulkAction('{{ $type }}', 'delete')"><i class="fas fa-trash-alt me-1"></i>Delete Selected</button>
                <form method="POST" action="{{ route('admin.trash.restore-all', $type) }}" class="d-inline" onsubmit="return confirm('Restore all {{ strtolower($group['label']) }}?')">
                    @csrf @method('PUT')
                    <button type="submit" class="btn btn-sm btn-outline-success"><i class="fas fa-undo me-1"></i>Restore All</button>
                </form>
                <x-confirm-delete
                    url="{{ route('admin.trash.empty', $type) }}"
                    label="Empty Trash"
                    title="Empty Trash?"
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    <thead class="table-light small">
                        <tr>
                            <th width="40"><input type="checkbox" class="select-all-{{ $type }}" onchange="toggleAll(this, '{{ $type }}')"></th>
                            <th width="60">ID</th>
                            <th>Title / Name</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><input type="checkbox" value="{{ $record->id }}" class="cb-{{ $type }}" onchange="updateSelected('{{ $type }}')"></td>
...
function bulkAction(type, action) {
    ...
    Swal.fire({
        ...
        const route = action === 'restore' ? '{{ route("admin.trash.bulk-restore") }}' : '{{ route("admin.trash.bulk-force-delete") }}';
        fetch(route, {
            method: 'POST',
            headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Content-Type': 'application/json' },
            body: JSON.stringify({ ids, model_type: type })
