...
<div class="crm-section-body">
    ...
    <div class="mt-4">
        <div class="task-box-header" style="font-size:.75rem;font-weight:700;color:#374151;padding-bottom:8px;border-bottom:2px solid #ede5f8;margin-bottom:10px;">✅ Completed & Cancelled ({{ $completedTasks->total() }})</div>
        <div id="completedTasksContainer">
            @php
                ...
                foreach ($completedTasks as $task) {
                    ...
                    if (!$date) { $groups['Other'][] = $task; continue; }
                    $diff = $today->diffInDays($date, false);
                    if ($diff === 0) $key = 'Today';
                    elseif ($diff === 1) $key = 'Yesterday';
                    elseif ($diff <= 7) $key = 'This Week';
                    else $key = 'Earlier';
                    $groups[$key][] = $task;
                }
            ...
            @forelse($order as $groupKey)
                @if (!isset($groups[$groupKey]) || empty($groups[$groupKey])) @continue @endif
                <div style="margin-bottom:12px;">
                    <div style="font-size:.65rem;font-weight:700;text-transform:uppercase;letter-spacing:.4px;color:#94a3b8;margin-bottom:6px;padding-left:2px;">{{ $groupKey }}</div>
                    <div style="display:flex;flex-direction:column;gap:4px;">
                        @foreach($groups[$groupKey] as $task)
                            @php
                                $isCompleted = $task->status === 'completed';
                                $isStageChange = $task->activity_type === 'stage_change';
                                $actionDate = $isCompleted ? $task->completed_at : ($task->cancelled_at ?? $task->updated_at);
                            @endphp
                            <div style="display:flex;gap:10px;background:#fff;border:1px solid #e8e5ee;border-radius:8px;padding:10px 12px;transition:box-shadow .12s;" onmouseover="this.style.boxShadow='0 2px 8px rgba(26,2,98,.06)'" onmouseout="this.style.boxShadow='none'">
                                {{-- Timeline indicator --}}
                                <div style="display:flex;flex-direction:column;align-items:center;width:14px;flex-shrink:0;padding-top:3px;">
                                    <div style="width:10px;height:10px;border-radius:50%;background:{{ $isCompleted ? '#820b5c' : '#ef4444' }};border:2px solid {{ $isCompleted ? '#ede5f8' : '#fef2f2' }};"></div>
                                ...
                                <div style="flex:1;min-width:0;">
                                    ...
                                    @if ($isCompleted && $task->completion_note)
                                        <div style="font-size:.88rem;font-weight:700;color:#1a0262;line-height:1.3;margin-bottom:2px;">{{ $task->completion_note }}</div>
                                    @elseif (!$isCompleted && $task->cancellation_note)
                                        <div style="font-size:.88rem;font-weight:700;color:#991b1b;line-height:1.3;margin-bottom:2px;">{{ $task->cancellation_note }}</div>
                                    @elseif ($isStageChange)
                                        <div style="font-size:.8rem;color:#334155;margin-bottom:2px;">{{ $task->description ?? 'Stage changed' }}</div>
                                    @endif
                                    {{-- Task subject (full, not truncated) --}}
                                    <div style="font-size:.75rem;color:#1a0262;font-weight:600;line-height:1.3;margin-bottom:2px;">{{ $task->subject }}</div>
                                    <div style="font-size:.65rem;color:#94a3b8;display:flex;align-items:center;gap:4px;flex-wrap:wrap;">
                                        <span style="display:inline-block;background:{{ $isCompleted ? '#ede5f8' : '#fee2e2' }};color:{{ $isCompleted ? '#1a0262' : '#991b1b' }};font-size:.55rem;font-weight:700;padding:0 6px;border-radius:3px;">{{ $isCompleted ? 'DONE' : 'CANCELLED' }}</span>
                                    </div>
                                    {{-- Meta: who assigned whom and who did it --}}
                                    <div style="font-size:.65rem;color:#64748b;margin-top:4px;line-height:1.4;">
                                        @if ($isStageChange)
                                            By {{ $task->creator?->name ?? 'Unknown' }} • {{ $actionDate?->format('d M Y, g:i A') }}
                                        @else
                                            @php
                                            ...
                                            @if ($isCompleted)
                                                ...
                                                @elseif ($doer)
                                                    <span style="color:#059669;"> • Completed by {{ $doerName }}</span>
                                                @endif
                                            @else
                                                @if ($doer)
                                                    <span style="color:#ef4444;"> • Cancelled by {{ $doerName }}</span>
                                                @endif
                                            @endif
                                            <span style="color:#94a3b8;"> • {{ $actionDate?->format('d M Y, g:i A') }}</span>
                                        @endif
                                    </div>
                                </div>
                                {{-- Actions --}}
                                <div style="display:flex;flex-direction:column;gap:3px;flex-shrink:0;">
                                    @if ($canEdit)
                                        <button onclick="{{ $isCompleted ? 'undoComplete' : 'undoCancel' }}({{ $task->id }})"
            ...
            @empty
                <div class="text-muted text-center py-3" style="font-size:.75rem">No completed or cancelled tasks yet.</div>
            @endforelse
            ...
            @if (isset($systemLogs) && $systemLogs->count() > 0)
                <div class="mt-3 pt-2" style="border-top:1px solid #e5e7eb;">
                    <div class="task-box-header" style="color:#6b7280;font-size:.75rem;">📋 System Updates ({{ $systemLogs->count() }})</div>
                    @foreach ($systemLogs as $log)
                        <div style="display:flex;gap:10px;background:#faf9fc;border:1px solid #e8e5ee;border-radius:8px;padding:8px 12px;margin-bottom:4px;">
                            ...
                            <div style="flex:1;min-width:0;">
                                ...
                                <div style="font-size:.72rem;color:#475569;white-space:pre-line;margin-top:2px;">{{ $log->content }}</div>
                                <div style="font-size:.65rem;color:#94a3b8;margin-top:2px;">
                                    {{ $log->creator?->name ?? 'Unknown' }} • {{ $log->created_at->format('d M Y, g:i A') }}
                                </div>
                            </div>
                            @if ($canEdit)
                                <x-confirm-delete
