...
<div class="text-center">
    <a href="{{ route('universities.create') }}" class="d-inline-block mx-auto p-3 text-white text-center fw-bold bg-secondary mb-3">+ Add New University</a>
</div>
@endif
@endauth
...
<table class="table table-bordered table-striped">
    ...
    <tbody>
        ...
        <tr>
            <td onclick="window.location='{{ route('universities.profile', $university->id) }}'" style="cursor:pointer;">{{ $university->id }}</td>
            <td onclick="window.location='{{ route('universities.profile', $university->id) }}'" style="cursor:pointer;">
                @if($university->university_logo)
                <img src="{{ asset('images/uni_logo/' . $university->university_logo) }}" width="40" alt="Logo">
                @endif
            </td>
            <td onclick="window.location='{{ route('universities.profile', $university->id) }}'" style="cursor:pointer;">{{ $university->name }}</td>
            <td onclick="window.location='{{ route('universities.profile', $university->id) }}'" style="cursor:pointer;">{{ $university->short_name ?? 'N/A' }}</td>
            ...
            <td class="text-center d-flex">
                <a href="{{ route('universities.edit', $university->id) }}" class="bg-gray-300 p-3 mx-1">Edit</a>
                <form method="POST" action="{{ route('universities.destroy', $university->id) }}" style="display:inline">
                    @csrf
                    @method('DELETE')
